...
print_banner() {
    ...
    echo "   ╚═╝   ╚═╝      ╚════╝  ╚═════╝  ╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═══╝╚═╝  ╚═╝╚══════╝"
    echo -e "${RESET}"
    echo "Terraform Run Recorder - Installer"
    echo
}
...
get_latest_version() {
    ...
    LATEST_RELEASE_URL="https://api.github.com/repos/Owloops/tfjournal/releases/latest"
...
get_download_url() {
    ...
    ASSET_PATTERN="tfjournal_${RELEASE_OS}_${RELEASE_ARCH}"
...
install_binary() {
    ...
    BINARY_PATH=$(find "${TMP_DIR}" -name "tfjournal*" -type f | head -1)
    if [[ -z "${BINARY_PATH}" ]]; then
        log ERROR "Failed to find the tfjournal binary in the download directory"
        exit 1
    fi

    cp "${BINARY_PATH}" "${INSTALL_DIR}/tfjournal"
    chmod +x "${INSTALL_DIR}/tfjournal"

    log INFO "tfjournal ${VERSION} has been installed to ${INSTALL_DIR}/tfjournal"
}

verify_installation() {
    if command -v "${INSTALL_DIR}/tfjournal" &>/dev/null; then
        log INFO "Installed version:"
        "${INSTALL_DIR}/tfjournal" --version
    fi

    echo
    echo "You can now run tfjournal from the command line."
    echo
    echo -e "${BLUE}Get started:${RESET}"
    echo "  tfjournal --help                    # Show help"
    echo "  tfjournal -- terraform apply        # Record a run"
    echo "  tfjournal                           # Open TUI"
    echo
    echo -e "${BLUE}Shell completions:${RESET}"
    echo "  # bash"
    echo "  source <(tfjournal completion bash)"
    echo
    echo "  # zsh"
    echo "  source <(tfjournal completion zsh)"
    echo
    echo "  # fish"
    echo "  tfjournal completion fish | source"
    echo
    echo -e "Documentation: ${GREEN}https://github.com/Owloops/tfjournal${RESET}"
}
